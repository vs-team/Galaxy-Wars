...
public class ControlGUIScript : MonoBehaviour {
	...
	void OnGUI() 
	{
		...
		if (IsPlaying != true)
		{
			...
			GUI.Label(new Rect(GBS.Position.x + Offset.x + LabelOffSet.x, GBS.Position.y + Offset.y + LabelOffSet.y - 40, 100, 25),"HPValue",LabelStyle);
			HPValue = GUI.HorizontalSlider(new Rect(GBS.Position.x + Offset.x , GBS.Position.y + Offset.y - 40, 180, 25), HPValue, 0.0F, 1F);
		}
	...
	void Update () 
	{
		...
		if (IsPlaying == true)
		{
			GBS.HPValue = ((Mathf.Sin (Time.time)/2f) + 0.5f) * 1.01f;
        }
		else
		{
			GBS.HPValue = HPValue;
