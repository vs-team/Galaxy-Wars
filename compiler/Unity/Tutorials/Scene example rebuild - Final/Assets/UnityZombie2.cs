...
public class UnityZombie2 : MonoBehaviour
{
  ...
  public bool Anim
  {
    get { return motor1.GetBool("Attack"); }
    set
    {
      motor1.SetBool("Attack", value);
    }
  }
  public Vector3 Dest
  {
    get { return Vector3.zero; }//Agent.destination; }
    set
    {
      //Agent.destination = value;
      //var zombieToCarDistance = Vector3.Distance(this.transform.position, value);
      //Agent.speed = 0.0f;// Mathf.Min(zombieToCarDistance * 4.5f, 25.0f) / 8.0f;
      //currentState = "Walk";
  ...
  public bool IsHitByForce
  {
    ...
    set
    {
      ...
      if (isHitByForce)
      {
        Life -= force * 1.0f * bodyPartMultiplier;
  ...
  public bool ApplyForceOnZombie //Sets the zombie as ragdoll and applies force in a direction
  {
    ...
    set
    {
      ...
      if ((collidedWithCar || dead) && applyForceOnZombie)
      {
        ...
        if (gameObject.name == hitCollider.GetComponentInParent<RagdollHelper>().name)
        {
          ...
          impacts.Add(collisionDirection * force * 500.0f); //impact direction also according to the ray
          impactEndTimes.Add(0.7f); //Apply the force for <float> length
