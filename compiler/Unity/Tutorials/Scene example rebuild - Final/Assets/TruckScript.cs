...
public class TruckScript : MonoBehaviour
{
  ...
  {
    get { return truckRigidBody.velocity; }
    set { truckRigidBody.velocity = value; }
  }
  ...
  void OnCollisionEnter(Collision collision)
  {
    ...
    else if (collision.transform.root.gameObject.layer == LayerMask.NameToLayer("ZombieCollisionLayer")) //Truck hits boxcollider of zombie
    {
      if (collision.gameObject.GetComponentInParent<UnityZombie2>().Anim)
      {
        CarHP2 -= 1.15f;
        CarHPChanged = 1.15f;
      }
      ...
      {
        ClusterInput.SetButton("TruckCollidedWithZombie", true);
      }
      //*/
    ...
    //else if (collision.transform.root.tag == "Zombiegroup" && collision.gameObject.layer == LayerMask.NameToLayer("Zombielayer"))
    //{
    //  Debug.Log("Zombie attacked truck");
    //}
    ...
    //    {
    //      //hitZombie.GetHit(-collision.relativeVelocity.normalized, collision.transform, collision.rigidbody,collision.collider, collision.relativeVelocity.magnitude / 30.0f, true, 2);
