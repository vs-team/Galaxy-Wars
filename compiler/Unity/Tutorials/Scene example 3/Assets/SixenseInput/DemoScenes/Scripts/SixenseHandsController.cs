    ...
    if ( bControllerActive )
		{
            ...
            if(Physics.Raycast(hand.transform.position, hand.transform.forward, out hitObject,100.0f, layermask))
            {
                if (hand.m_controller.GetButtonDown(SixenseButtons.TRIGGER))
                {
                  hitObject.transform.gameObject.GetComponent<UnityZombie>().shot = true;
                }
      }
		}
